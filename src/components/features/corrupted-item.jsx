...
export const CorruptedItem = ({ type, service, onDelete }) => (
  <Item variant="outline" className="dark:bg-muted/30 w-full gap-2.5 opacity-50">
    <ItemMedia variant="icon" className="bg-muted">
      <CircleAlertIcon />
    </ItemMedia>
    <ItemContent className="gap-0">
      <ItemDescription className="text-muted-foreground pt-1 text-xs">
        {type} for <strong className="font-semibold">{service}</strong> is corrupted and cannot be
        read.
      </ItemDescription>
    </ItemContent>
    <ItemActions>
